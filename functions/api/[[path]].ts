import { hasValidSessionCookie, jsonResponse, proxyApiRequest, withRobotsNoIndex, withSecurityHeaders, type PagesEnv } from '../_shared/authProxy';
...
export const onRequest = async (context: { request: Request; env: PagesEnv; next: () => Promise<Response> }) => {
  ...
  return withRobotsNoIndex(withSecurityHeaders(await proxyApiRequest(context.request, context.env)));
};
